...
/**
 * Encodes {@link DialogMessagesWrapper} to JSON string
 * @author VetalZloy
 *
 */
@Component
public class DialogMessagesEncoder implements Encoder.Text<DialogMessagesWrapper>{
	...
	@Override
	public String encode(DialogMessagesWrapper wrapper) throws EncodeException {
		JsonArrayBuilder arrayBuilder = Json.createArrayBuilder();
		List<DialogMessage> list = wrapper.getMessages();
		Collections.sort(list);
		
		for(DialogMessage message: list) {
		...
		long earliestId = -1;
		if(list.size() > 0) earliestId = list.get(0).getMessageId();
